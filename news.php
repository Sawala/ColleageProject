...
{
	$query_RecNews_part = "SELECT * FROM `news` WHERE `newsid` = '".$_GET["newsid"]."'";
	$RecNews_part = mysql_query($query_RecNews_part);
	$row_RecNews_part=mysql_fetch_assoc($RecNews_part);
}
$query_RecNews = "SELECT * FROM `news` ORDER BY `newstime` DESC";
$RecNews = mysql_query($query_RecNews);
...
<div id="Menu">
   <ul>
	...
	<div id="EachContent">
		...
		if(isset($_GET["newsid"]))
		{
			echo "第".$_GET["newsid"]."條站方公告<br>";
			echo "公告標題：".$row_RecNews_part["title"]."<br>";
			echo "日期：".$row_RecNews_part["newstime"]."<br>";
			echo "公告事項：".$row_RecNews_part["contents"]."<br>";
		}
		?>
	</div>
	</div>
